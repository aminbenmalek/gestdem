...
import { saveProduct, deleteProductAsync } from "../src/store/catalogSlice";
...
const ProductCatalog: React.FC = () => {
  ...
  const confirmDelete = () => {
    if (productToDelete) {
      dispatch(deleteProductAsync(productToDelete.id) as any);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingProduct) {
      const diffs: string[] = [];
      if (editingProduct.label !== label)
        diffs.push(`Désignation: ${editingProduct.label} -> ${label}`);
      if (editingProduct.code !== code)
        diffs.push(`Code: ${editingProduct.code} -> ${code}`);
      if (editingProduct.minStockLevel !== minStock)
        diffs.push(`Seuil: ${editingProduct.minStockLevel} -> ${minStock}`);
    ...
    dispatch(saveProduct(product) as any);
    setIsFormOpen(false);
  };
  ...
  const processedProducts = useMemo(() => {
    const filtered = products.filter((p) => {
      ...
      const matchesTax =
        taxFilter === "all" || p.defaultTaxRate.toString() === taxFilter;
      const matchesFodec =
        fodecFilter === "all" ||
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-blue-600"
              />
            </div>
            <div className="flex gap-4">
              ...
              <div className="flex flex-col items-center">
                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1 tracking-widest">
                  FODEC
                </label>
                <input
                  type="checkbox"
                  checked={fodec}
                  onChange={(e) => setFodec(e.target.checked)}
            ...
            <div className="lg:col-span-5 flex justify-end gap-2 pt-4">
              <button
                type="submit"
                className="px-8 py-2 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 shadow-md flex items-center gap-2"
              >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <table className="w-full">
          <thead className="bg-slate-50">
            <tr>
              <th
                className="px-6 py-4 text-left text-xs font-bold text-slate-400 uppercase tracking-widest cursor-pointer"
              ...
              >
                Désignation {renderSortIcon("label")}
              </th>
              <th className="px-6 py-4 text-center text-xs font-bold text-slate-400 uppercase tracking-widest">
                Seuil Alerte
              </th>
              <th
                className="px-6 py-4 text-right text-xs font-bold text-slate-400 uppercase tracking-widest cursor-pointer"
                onClick={() => toggleSort("defaultUnitPrice")}
              >
                Prix de base {renderSortIcon("defaultUnitPrice")}
              </th>
            ...
            {paginatedProducts.map((p) => (
              ...
              >
                ...
                <td className="px-6 py-4 text-right font-medium">
                  {p.defaultUnitPrice.toFixed(3)} TND
                </td>
