...
import { LayoutDashboard, FileText, Users, PlusCircle, LogOut, Package, MapPin, ArrowLeftRight } from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children, activeView, setView }) => {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      <aside className="w-64 bg-slate-900 text-white flex flex-col fixed h-full transition-all duration-300 shadow-2xl z-40">
        <div className="p-6">
          <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2">
            <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center text-sm" aria-hidden="true">G</div>
            GestionPro
          </h1>
        </div>

        <nav className="flex-1 px-4 py-4 space-y-1" aria-label="Navigation principale">
          {navItems.map((item) => {
      ...
      {/* Main Content */}
      <main id="main-content" className="flex-1 ml-64 p-8 outline-none" tabIndex={-1}>
        <header className="mb-8 flex justify-between items-center">
          <div>
            <h2 className="text-3xl font-bold text-slate-900">
              {navItems.find(n => n.id === activeView)?.label}
            </h2>
            <p className="text-slate-600 mt-1 font-medium">Gestionnaire de flux d'achat intelligent</p>
          </div>
          <div className="flex items-center gap-4">
            <div className="bg-white p-2 rounded-full border border-slate-200 shadow-sm px-4 flex items-center gap-2" role="status" aria-live="polite">
              <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" aria-hidden="true"></div>
              <span className="text-xs font-bold text-slate-700 uppercase tracking-wider">Mode Connecté</span>
            </div>
          </div>
        </header>
        {children}
