...
import { saveSupplier, deleteSupplierAsync } from "../src/store/suppliersSlice";
...
const SupplierManagement: React.FC = () => {
  ...
  const [viewingHistorySupplier, setViewingHistorySupplier] =
  ...
  const confirmDelete = () => {
    if (supplierToDelete) {
      dispatch(deleteSupplierAsync(supplierToDelete.id) as any);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingSupplier) {
      const diffs: string[] = [];
      if (editingSupplier.name !== name)
        diffs.push(`Nom: ${editingSupplier.name} -> ${name}`);
      if (editingSupplier.category !== category)
        diffs.push(`Catégorie: ${editingSupplier.category} -> ${category}`);
      if (editingSupplier.taxId !== taxId)
        diffs.push(`Matricule: ${editingSupplier.taxId} -> ${taxId}`);
    ...
    dispatch(saveSupplier(supplier) as any);
    setIsFormOpen(false);
  };
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh] animate-in zoom-in-95 duration-200">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {viewingHistorySupplier.history &&
              viewingHistorySupplier.history.length > 0 ? (
                <div className="space-y-6">
                  {viewingHistorySupplier.history
                    ...
                    .map((entry) => (
                      ...
                      >
                        <div
                          className={`absolute left-0 top-1 w-6 h-6 rounded-full border-4 border-white shadow-sm flex items-center justify-center ${entry.action === "Création" ? "bg-green-500" : "bg-blue-500"}`}
                        >
                        ...
                        <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                          <div className="flex justify-between items-start mb-2">
                            <span
                              className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${entry.action === "Création" ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700"}`}
                            >
                              {entry.action}
                            </span>
          ...
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 bg-slate-50 flex gap-3">
              <button
                ...
                className="flex-1 py-3 bg-white border text-slate-600 font-bold rounded-2xl"
              >
                Annuler
              </button>
              <button
          ...
          >
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="flex gap-1">
                <button
                  onClick={() => setViewingHistorySupplier(s)}
        ...
        {filteredSuppliers.length === 0 && (
          <div className="col-span-full py-24 bg-white rounded-3xl border-2 border-dashed border-slate-100 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-slate-400 max-w-xs mx-auto">
              Vérifiez vos critères de recherche ou ajoutez un nouveau
