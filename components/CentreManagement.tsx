...
import { addOrUpdateCentre, removeCentre } from '../src/store/centresSlice';
import { Centre, HistoryEntry } from '../types';
import { storageService } from '../services/storageService';
import { Plus, Trash2, Search, Edit2, X, Save, MapPin, Building2, AlertTriangle, History, User, ChevronLeft, ChevronRight } from 'lucide-react';
...
const CentreManagement: React.FC = () => {
  ...
  const [viewingHistoryCentre, setViewingHistoryCentre] = useState<Centre | null>(null);
  ...
  const confirmDelete = () => {
    if (centreToDelete) {
      dispatch(removeCentre(centreToDelete.id));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingCentre) {
      const diffs: string[] = [];
      if (editingCentre.name !== name) diffs.push(`Nom: ${editingCentre.name} -> ${name}`);
      if (editingCentre.location !== location) diffs.push(`Emplacement: ${editingCentre.location} -> ${location}`);
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
          ...
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh] animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xl font-bold text-slate-900">Suivi du Centre</h3>
                  <p className="text-xs text-slate-500 font-medium">{viewingHistoryCentre.name}</p>
                </div>
              </div>
              <button onClick={() => setViewingHistoryCentre(null)} className="p-2 hover:bg-slate-200 rounded-full">
                <X size={20} className="text-slate-500" />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-6">
              {viewingHistoryCentre.history && viewingHistoryCentre.history.length > 0 ? (
                <div className="space-y-6">
                  {viewingHistoryCentre.history.slice().reverse().map((entry) => (
                    <div key={entry.id} className="relative pl-8 before:absolute before:left-[11px] before:top-2 before:bottom-[-24px] before:w-0.5 before:bg-slate-100 last:before:hidden">
                      <div className={`absolute left-0 top-1 w-6 h-6 rounded-full border-4 border-white shadow-sm flex items-center justify-center ${entry.action === 'Création' ? 'bg-green-500' : entry.action === 'Réception Stock' ? 'bg-blue-600' : 'bg-amber-500'}`}>
                        {entry.action === 'Création' ? <Plus size={10} className="text-white" /> : entry.action === 'Réception Stock' ? <Building2 size={10} className="text-white" /> : <Edit2 size={10} className="text-white" />}
                      </div>
                      <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                        <div className="flex justify-between items-start mb-2">
                          <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${entry.action === 'Création' ? 'bg-green-100 text-green-700' : entry.action === 'Réception Stock' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700'}`}>
                            {entry.action}
                          </span>
            ...
            <div className="p-4 bg-slate-50 text-right">
              <button onClick={() => setViewingHistoryCentre(null)} className="px-6 py-2 bg-white border rounded-xl font-bold">Fermer</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-8 text-center">
              ...
              <p className="text-slate-500 leading-relaxed">
                Voulez-vous supprimer le centre <span className="font-bold text-slate-800">"{centreToDelete.name}"</span> ?
                Cela pourrait affecter la visibilité des anciens bons de commande associés.
              </p>
            </div>
      ...
      {isFormOpen && (
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 animate-in slide-in-from-top-4 duration-300">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-400 uppercase mb-1 tracking-widest">Nom du Centre</label>
              <input 
                type="text" 
                value={name}
            ...
            <div>
              <label className="block text-xs font-bold text-slate-400 uppercase mb-1 tracking-widest">Gouvernorat / Ville</label>
              <input 
                type="text" 
                value={location}
            ...
            <div className="md:col-span-2">
              <label className="block text-xs font-bold text-slate-400 uppercase mb-1 tracking-widest">Description (Optionnel)</label>
              <textarea 
                value={description}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {paginatedCentres.map(c => (
          <div key={c.id} className="group bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-all relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="flex gap-1">
                <button onClick={() => setViewingHistoryCentre(c)} className="p-2 text-slate-400 hover:text-indigo-600 transition-colors">
                  <History size={16} />
                </button>
                <button onClick={() => handleEdit(c)} className="p-2 text-slate-400 hover:text-blue-600 transition-colors">
                  <Edit2 size={16} />
                </button>
                <button onClick={() => setCentreToDelete(c)} className="p-2 text-slate-400 hover:text-red-600 transition-colors">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
            ...
            <p className="text-xs text-slate-400 leading-relaxed line-clamp-2">
              {c.description || "Aucune description fournie pour ce centre de coût."}
            </p>
        ...
        {filteredCentres.length === 0 && (
          <div className="col-span-full py-20 bg-white rounded-2xl border-2 border-dashed border-slate-100 flex flex-col items-center justify-center gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-slate-400">Essayez un autre terme ou créez-en un nouveau.</p>
            </div>
          </div>
        )}
      </div>
