...
import { saveMovement, refreshMovements } from "../src/store/stockSlice";
import {
  StockMovement,
  StockMovementItem,
  Centre,
...
const StockManagement: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "list" | "analytics" | "alerts" | "stock"
  >("stock");
  const [chartProductId, setChartProductId] = useState<string>(
    products[0]?.id || "",
  );
  const [chartCentreId, setChartCentreId] = useState<string>(
    centres[0]?.id || "",
  );
  ...
  const findProductLastKnownLocation = (productId: string): string | null => {
    const sortedOrders = [...ordersFromStore].sort(
  ...
  const handleAddItem = (p: Product) => {
    ...
    if (!sourceCentreId) {
      ...
      if (lastLocationId) {
        ...
        setAutoSelectInfo(
          `Le centre "${centreName}" a été sélectionné car l'article y a été localisé en dernier.`,
        );
    ...
    setMvtItems([
      ...mvtItems,
      { productId: p.id, label: p.label, quantity: 1 },
    ]);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!sourceCentreId || !destinationCentreId)
      return alert(
        "Veuillez sélectionner les centres d'origine et de destination.",
      );
    if (sourceCentreId === destinationCentreId)
      return alert(
        "Le centre de destination doit être différent du centre d'origine.",
      );

    if (mvtItems.length === 0)
      return alert("Veuillez ajouter au moins un article.");

    let hasError = false;
    mvtItems.forEach((item) => {
      const available = storageService.getProductStock(
        item.productId,
        sourceCentreId,
        movements,
      );
      if (item.quantity <= 0) {
        ...
        alert(
          `Stock insuffisant pour "${item.label}". Disponible: ${available}`,
        );
        hasError = true;
      }
    });
  ...
  const handleConfirmTransfer = async () => {
    ...
    try {
      await dispatch(saveMovement(newMvt) as any);
      setIsConfirmModalOpen(false);
      setIsFormOpen(false);
  ...
  const getSourceDisplay = (id: string) => {
    if (id.startsWith("SUPPLIER:")) {
      ...
      return {
        name: supplier?.name || "Fournisseur Externe",
        type: "Achat",
        icon: ShoppingCart,
        color: "emerald",
      };
  ...
  const getCentreName = (id: string) =>
    centres.find((c) => c.id === id)?.name || "Inconnu";
  ...
  const chartData = useMemo(() => {
    ...
    const relevantMovements = movements
      ...
      .filter(
        (m) =>
          m.sourceCentreId === chartCentreId ||
          m.destinationCentreId === chartCentreId,
      )
    ...
    relevantMovements.forEach((m) => {
      ...
      if (item) {
        if (m.destinationCentreId === chartCentreId)
          cumulativeStock += item.quantity;
        if (m.sourceCentreId === chartCentreId)
          cumulativeStock -= item.quantity;
        historyMap.set(m.date, cumulativeStock);
      }
    });

    return Array.from(historyMap.entries()).map(([date, stock]) => ({
  ...
  const availableProductsForSource = useMemo(() => {
    ...
    return products.filter(
      (p) =>
        storageService.getProductStock(p.id, sourceCentreId, movements) > 0,
    );
  ...
  const filteredMovements = useMemo(() => {
    return movements.filter((m) => {
      const matchesSearch =
        ...
        getCentreName(m.destinationCentreId)
          .toLowerCase()
          .includes(searchTerm.toLowerCase()) ||
        getSourceDisplay(m.sourceCentreId)
          .name.toLowerCase()
          .includes(searchTerm.toLowerCase());

      const matchesProduct =
        !filterProductId ||
        m.items.some((item) => item.productId === filterProductId);
      const matchesCentre =
        !filterCentreId ||
        m.sourceCentreId === filterCentreId ||
        m.destinationCentreId === filterCentreId;

      const mvtDate = new Date(m.date);
      const matchesStartDate =
        !mvtStartDate || mvtDate >= new Date(mvtStartDate);
  ...
  const currentStockLevel = storageService.getProductStock(
    chartProductId,
    chartCentreId,
    movements,
  );
  ...
  return (
    <div className="space-y-6">
      {/* Tabs */}
      <div
        className="flex border-b border-slate-200 gap-8 px-2"
        role="tablist"
      ...
      >
        ...
        >
          ...
          {activeTab === "stock" && (
            <div
              className="absolute bottom-0 left-0 w-full h-1 bg-blue-600 rounded-t-full"
              aria-hidden="true"
            ></div>
          )}
        </button>
          ...
          <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden">
            <div className="p-8 border-b border-slate-50 flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-slate-500 font-medium">
                  État des stocks par article et par centre de coût.
                </p>
              </div>
            ...
            <div className="overflow-x-auto">
              <table
                ...
                aria-label="Tableau d'inventaire global"
              >
                <thead>
                  <tr className="bg-slate-50">
                    <th
                      scope="col"
                      className="p-6 text-left text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] sticky left-0 bg-slate-50 z-10 border-b border-slate-100"
                    >
                      Article
                    </th>
                    {centres.map((c) => (
                      <th
                        key={c.id}
                        scope="col"
                        className="p-6 text-center text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] border-b border-slate-100 min-w-[150px]"
                      >
                        {c.name.split(" - ")[0]}
                      </th>
                    ))}
                    <th
                      scope="col"
                      className="p-6 text-right text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] border-b border-slate-100"
                    >
                      Total
                    </th>
                  </tr>
                  ...
                  {filteredProducts.map((p) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {centres.map((c) => {
                          const stock = storageService.getProductStock(
                            p.id,
                            c.id,
                            movements,
                          );
                          ...
                          const isLow =
                            p.minStockLevel && stock <= p.minStockLevel;
                          const isCritical =
                            p.minStockLevel && stock <= p.minStockLevel / 2;

                          return (
                            <td key={c.id} className="p-6 text-center">
                              <span
                                className={`text-lg font-black ${isCritical ? "text-red-700" : isLow ? "text-amber-600" : "text-slate-900"}`}
                              >
                                {stock}
                              </span>
                              {isLow && (
                                <div className="mt-1">
                                  <span
                                    className={`text-[8px] font-black uppercase px-1.5 py-0.5 rounded ${isCritical ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}
                                  >
          ...
          <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
            <div className="flex justify-between items-center mb-10">
              <div>
                ...
                <p className="text-sm text-slate-500 font-medium">
                  Réapprovisionnement suggéré pour les articles sous le seuil de
                  sécurité.
                </p>
              </div>
              <div
                className="flex items-center gap-2 px-4 py-2 bg-red-50 text-red-700 rounded-xl font-bold text-xs"
                role="status"
                aria-live="polite"
              >
              ...
              {stockAlerts.length > 0 ? (
                stockAlerts.map((alert, idx) => {
                  const percentage = Math.min(
                    100,
                    (alert.stock / (alert.product.minStockLevel || 1)) * 100,
                  );
                  return (
                    <div
                      key={idx}
                      className={`group bg-white p-6 rounded-3xl border ${alert.isCritical ? "border-red-100 bg-red-50/10" : "border-slate-100"} hover:shadow-xl transition-all duration-300 relative overflow-hidden`}
                    >
                      <div
                        className={`absolute top-0 right-0 w-24 h-24 -mr-8 -mt-8 rounded-full opacity-5 ${alert.isCritical ? "bg-red-600" : "bg-amber-600"}`}
                        aria-hidden="true"
                      ></div>

                      <div className="flex justify-between items-start mb-6">
                        <div
                          className={`w-12 h-12 rounded-2xl flex items-center justify-center ${alert.isCritical ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}
                          aria-hidden="true"
                        >
                          <Package size={24} />
                        </div>
                        <div className="text-right">
                          <span
                            className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-lg ${alert.isCritical ? "bg-red-700 text-white" : "bg-amber-600 text-white"}`}
                          >
                      ...
                      <div className="space-y-4 mb-8">
                        <div className="flex justify-between items-end">
                          <div>
                            ...
                            <p
                              className={`text-3xl font-black ${alert.isCritical ? "text-red-600" : "text-slate-900"}`}
                            >
                              {alert.stock}
                            </p>
                          </div>
                ...
                <div className="col-span-full py-20 flex flex-col items-center justify-center text-slate-300 gap-4">
                  ...
                  <p className="font-black text-lg text-slate-500">
                    Stocks en parfaite conformité
                  </p>
                  <p className="text-sm font-medium">
                    Aucun article n'est actuellement sous son seuil d'alerte.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm col-span-1 lg:col-span-1 space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  <label
                    htmlFor="analytics-product-select"
                ...
                <div>
                  <label
                    htmlFor="analytics-centre-select"
                    className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2"
                  >
                    Centre de Coût
                  </label>
              ...
              <div className="pt-6 border-t border-slate-50">
                <div className="bg-blue-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-200">
                  ...
                  <div className="mt-4 flex items-center gap-2 text-[10px] font-bold">
                    <div
                      className={`w-2 h-2 rounded-full ${currentStockLevel > (selectedProductInfo?.minStockLevel || 0) ? "bg-green-400" : "bg-red-400"}`}
                      aria-hidden="true"
                    ></div>
                    {currentStockLevel >
                    (selectedProductInfo?.minStockLevel || 0)
                      ? "NIVEAU OPTIMAL"
                      : "ALERTE RÉAPPRO"}
                  </div>
                </div>
            ...
            <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm lg:col-span-3">
              <div className="flex justify-between items-center mb-8">
                <div>
                  <h3 className="text-xl font-black text-slate-900 tracking-tighter">
                    Évolution Temporelle du Stock
                  </h3>
                  <p className="text-xs text-slate-500 font-medium">
                    Variation cumulative basée sur les entrées et sorties de
                    stock.
                  </p>
                </div>
              </div>
              ...
              <div
                ...
                aria-label="Graphique d'évolution temporelle du stock"
              >
                {chartData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      {selectedProductInfo?.minStockLevel && (
                        <ReferenceLine
                          y={selectedProductInfo.minStockLevel}
                          stroke="#ef4444"
                          strokeDasharray="3 3"
                          label={{
                            position: "right",
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-slate-300 gap-4">
                    ...
                    <p className="font-bold text-sm text-slate-500">
                      Aucune donnée historique pour cette sélection.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="flex justify-between items-center mb-8 pb-6 border-b border-slate-50">
                <div>
                  ...
                  <p className="text-slate-500 font-medium">
                    Gestion des stocks inter-centres.
                  </p>
                </div>
                <button
              ...
              >
                <div className="space-y-8">
                  <div className="grid grid-cols-2 gap-4 relative">
                    <div>
                      <label
                        htmlFor="source-centre"
                        className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3 px-2"
                      >
                        Origine
                      </label>
                      <select
                        ...
                        onChange={(e) => {
                          setSourceCentreId(e.target.value);
                          setAutoSelectInfo(null);
                          setMvtItems([]);
                        }}
                    ...
                    <div className="flex items-center justify-center pt-8">
                      <div
                        className="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center text-blue-600 shadow-sm"
                        aria-hidden="true"
                      >
                        <ArrowRight size={20} />
                      </div>
                ...
                <div className="flex flex-col h-full bg-slate-50 p-8 rounded-[2rem] border border-slate-200 shadow-inner">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    >
                      ...
                      ) : availableProductsForSource.length > 0 ? (
                        <>
                          <option value="">+ Ajouter un article</option>
                          {availableProductsForSource.map((p) => (
                            <option key={p.id} value={p.id}>
                              {p.label} (Dispo:{" "}
                              {storageService.getProductStock(
                                p.id,
                                sourceCentreId,
                                movements,
                              )}
                  ...
                  <div
                    className="flex-1 space-y-4 overflow-y-auto pr-2 max-h-[400px]"
                    role="list"
                    aria-label="Articles sélectionnés"
                  >
                    {mvtItems.map((item) => (
                      <div
                        key={item.productId}
                        className="bg-white p-5 rounded-2xl border border-slate-200 flex justify-between items-center"
                        role="listitem"
                      >
            ...
            >
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  <th
                    scope="col"
                    className="p-6 text-left text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]"
                  >
                    Identifiant
                  </th>
                  <th
                  ...
                  <th
                    scope="col"
                    className="p-6 text-center text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]"
                  >
                    Détails
                  </th>
                  <th
                  ...
                  <th
                    scope="col"
                    className="p-6 text-right text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]"
                  >
                    Actions
                  </th>
                </tr>
                ...
                {paginatedMovements.map((m) => (
                  ...
                  >
                    <td className="p-6">
                      <div className="flex items-center gap-4">
                        <div
                          className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center group-hover:bg-slate-900 group-hover:text-white"
                          aria-hidden="true"
                        >
                          {m.reference.startsWith("IN-") ? (
                        ...
                        <div>
                          ...
                          <span className="text-xs text-slate-500 font-bold">
                            {new Date(m.date).toLocaleDateString("fr-TN")}
                          </span>
                        </div>
                      </div>
                    </td>
                    <td className="p-6">
                      <div className="flex items-center gap-4">
                        <span className="font-bold text-slate-800 text-sm">
                          {getSourceDisplay(m.sourceCentreId).name}
                        </span>
                        <ArrowRight
                    ...
                    <td className="p-6">
                      <span
                        className="text-xs text-slate-500 font-medium line-clamp-1 max-w-[150px]"
                        title={m.notes || ""}
                      >
                        {m.notes || "-"}
                      </span>
        ...
        >
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-8 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
              <div className="flex items-center gap-3">
                <div
                  className="w-10 h-10 bg-amber-100 text-amber-700 rounded-2xl flex items-center justify-center"
                  aria-hidden="true"
                >
            ...
            <div className="p-8 space-y-6">
              <div className="flex items-center justify-between p-4 bg-slate-100 rounded-2xl border border-slate-200">
                ...
                <div className="text-center flex-1">
                  ...
                  <p className="font-bold text-blue-700">
                    {getCentreName(destinationCentreId)}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-2">
                  Résumé des articles ({mvtItems.length})
                </p>
                <div
              ...
              {notes && (
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 italic text-slate-600 text-xs">
                  <span className="font-black uppercase not-italic block mb-1 text-[9px] text-slate-500">
                    Note:
                  </span>
                  {notes}
                </div>
            ...
            <div className="flex-1 overflow-y-auto p-10 print:p-0">
              <div id="print-area" className="space-y-10">
                <div className="flex justify-between items-start border-b-2 border-slate-900 pb-8">
                  <div>
                    <h1 className="text-4xl font-black text-slate-900 mb-2 uppercase">
                      BON DE MOUVEMENT
                    </h1>
                  ...
                  <div className="text-right">
                    <p className="font-black text-slate-900 text-xl tracking-tighter">
                      DATE:{" "}
                      {new Date(viewingMovement.date).toLocaleDateString(
                        "fr-FR",
                      )}
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-12">
                  <div className="p-6 bg-slate-50 rounded-2xl border border-slate-200">
                    ...
                    <p className="text-xl font-black text-slate-900 mb-1">
                      {getSourceDisplay(viewingMovement.sourceCentreId).name}
                    </p>
                  </div>
                  <div className="p-6 bg-blue-50 rounded-2xl border border-blue-200">
                    <h4 className="text-[10px] font-black text-blue-700 uppercase tracking-[0.2em] mb-4">
                      Unité Réceptionnaire
                    </h4>
                    <p className="text-xl font-black text-slate-900 mb-1">
                      {getCentreName(viewingMovement.destinationCentreId)}
                    </p>
                  </div>
                </div>
                <table
                  className="w-full border-collapse"
                  aria-label="Détails des articles du mouvement"
                >
                  <thead>
                    <tr className="bg-slate-900 text-white">
                      <th
                        scope="col"
                        className="p-4 text-left font-black uppercase text-[10px] tracking-[0.2em]"
                      >
                        Désignation
                      </th>
                      <th
                        scope="col"
                        className="p-4 text-center font-black uppercase text-[10px] tracking-[0.2em] w-32"
                      >
                        Unité
                      </th>
                      <th
                        scope="col"
                        className="p-4 text-right font-black uppercase text-[10px] tracking-[0.2em] w-32"
                      >
                        Quantité
                      </th>
                    </tr>
